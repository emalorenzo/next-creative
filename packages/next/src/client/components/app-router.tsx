...
const NAV_ID_HISTORY_STATE_KEY = '__PRIVATE_NEXTJS_INTERNALS_NAV_ID'

function readHistoryNavigationId(state: any): number | null {
  const value = state?.[NAV_ID_HISTORY_STATE_KEY]
...
function HistoryUpdater({
  ...
  useInsertionEffect(() => {
    ...
    if (globalMutable.currentNavigationId === undefined) {
      globalMutable.currentNavigationId =
        readHistoryNavigationId(window.history.state) ?? 0
    }
    ...
    const nextNavigationId =
      shouldPush && globalMutable.currentNavigationId !== undefined
        ? globalMutable.currentNavigationId + 1
        : globalMutable.currentNavigationId
    ...
    const historyState = {
      ...
      [NAV_ID_HISTORY_STATE_KEY]: nextNavigationId,
    }
    if (shouldPush) {
    ...
    globalMutable.currentNavigationId = nextNavigationId
  }, [appRouterState])
...
function copyNextJsInternalHistoryState(data: any) {
  ...
  const navigationId = readHistoryNavigationId(currentState)
  if (navigationId !== null) {
    data[NAV_ID_HISTORY_STATE_KEY] = navigationId
  }
...
function Router({
  ...
  const pendingPushAtRender = state.pushRef.pendingPush
  const previousCanonicalUrlRef = useRef(canonicalUrl)
  const pendingTraverseDirectionRef =
    useRef<NavigationTransitionDirection | null>(null)
  const [navigationTransition, setNavigationTransition] =
    useState<NavigationTransition | null>(null)
  ...
  useEffect(() => {
    const from = previousCanonicalUrlRef.current
    if (from === canonicalUrl) {
      pendingTraverseDirectionRef.current = null
      return
    }

    const traverseDirection = pendingTraverseDirectionRef.current
    pendingTraverseDirectionRef.current = null
    ...
    let direction: NavigationTransitionDirection = 'none'
    if (traverseDirection !== null) {
      direction = traverseDirection
    } else if (
      action === 'push' &&
      toPathname(from) !== toPathname(canonicalUrl)
    ) {
      direction = 'forward'
    }
  ...
  }, [canonicalUrl, pendingPushAtRender])
  ...
  useEffect(() => {
    ...
    function handlePageShow(event: PageTransitionEvent) {
      ...
      globalMutable.currentNavigationId =
        readHistoryNavigationId(window.history.state) ??
        globalMutable.currentNavigationId
  ...
  useEffect(() => {
    ...
    const onPopState = (event: PopStateEvent) => {
      ...
      const nextNavigationId =
        readHistoryNavigationId(event.state) ??
        globalMutable.currentNavigationId
      const currentNavigationId =
        globalMutable.currentNavigationId ?? nextNavigationId

      let traversalDirection: NavigationTransitionDirection = 'none'
      if (
        nextNavigationId !== undefined &&
        currentNavigationId !== undefined &&
        nextNavigationId < currentNavigationId
      ) {
      ...
      globalMutable.currentNavigationId = nextNavigationId
      pendingTraverseDirectionRef.current = traversalDirection
  ...
  return (
    <>
      ...
      >
        <NavigationTransitionContext.Provider value={navigationTransition}>
